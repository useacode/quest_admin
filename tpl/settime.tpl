        ...
        <div class="row-time-wrap">           
            ...
            <div class="col-time-left  col-time-left--top">
                <div class="row-time" ng-repeat="item in data">
                    ...
                    <div ng-repeat="time in item.times" ng-class="{'time': true, 'item-wait': time.status == 'wait', 'item-multi': time.status == 'multi', 'item-close': time.status == 'close'}" title="{{getTitle(time.status)}}" ng-click="select(time)" >{{time.time}}</div>
                </div>
        </div>
    </div>
